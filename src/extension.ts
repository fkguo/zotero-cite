...
function minimizeZotero(): string {
...
function validateBibName(bibName: string): void {
  if (bibName.length < 5 || path.extname(bibName) !== ".bib") {
    throw new Error(t("error.invalidBibName"));
  }
}

function applyBibTemplateVariables(template: string, filePath: string): string {
...
function resolveBibPath(currentFileUri: vscode.Uri, bibNameTemplate: string): vscode.Uri {
 ...
 * 获取文档中引用的键列表
 */
function getDocumentCiteKeys(): string[] {
 ...
 */
function insertText(text: string, location = -1): void {
  const editor = getActiveEditor();
...
async function insertTextAsync(text: string, location = -1): Promise<void> {
  const editor = getActiveEditor();
 ...
 */
async function insertMarkdownBibliography(citeKey: string): Promise<void> {
  ...
  try {
    ...
    const data = response.data as { error?: { message?: string }; result?: string };
    if (data.error) {
 ...
 */
function insertCiteKeys(keyList: string[]): void {
  const editor = getActiveEditor();
 ...
 */
async function getItemGroupName(key: string): Promise<string> {
  const payload = JSON.stringify({
    jsonrpc: "2.0",
    method: "item.search",
    params: [key],
  ...
  const data = response.data as {
    error?: { message?: string };
    result?: Array<{ "citation-key": string; library: string }>;
  };
 ...
 */
async function getBibliographyInGroup(keys: string[], groupId: string): Promise<string> {
  const payload = JSON.stringify({
    jsonrpc: "2.0",
    method: "item.export",
 ...
 */
async function getBibliography(keys: string[]): Promise<string> {
 ...
 */
function getKeyEnvOffset(): number | null {
  const editor = getActiveEditor();
 ...
 * 获取鼠标前后目标长度的文字内容
 */
function getCursorRoundText(length = 50): CursorRoundText | undefined {
  const editor = getActiveEditor();
...
async function getBibtexFromZotero(citeKey: string): Promise<string | null> {
  try {
    ...
    showErrorMessage(t("error.noResultFromZotero"));
    return null;
  } catch (error) {
    showErrorMessage(
      t("error.fetchFromZoteroFailed", {
        message: errorToMessage(error),
      })
    );
    return null;
...
async function updateBibEntries(): Promise<void> {
  ...
  try {
    ...
    if (missingKeys.length > 0) {
      ...
      if (selection === t("action.showList")) {
        outputChannel.show(true);
      } else if (selection === t("action.copyList")) {
